          ...
          <Form inline class="flex-center-start" v-if="Retract==true">
            ...
            <FormItem label="有效状态:" prop="ValidStateList">
              <Select style="width:150px" v-model="statues">
                <Option v-for="item in ValidStateList" :value="item.value" :key="item.value">{{ item.label }}</Option>
              </Select>
            </FormItem>
          ...
          <Form ref="ModifyDate" :model="ModifyDate" :rules="ruleValidate">
            <FormItem label="物资种类" prop="MaterialTypes">
              <Select style="width: 10rem" v-model="ModifyDate.typeDicId">
                <Option v-for="item in MaterialTypes" :value="item.dicId" :key="item.dicId">{{ item.dicName }}</Option>
              </Select>
            </FormItem>
...
export default {
  data() {
    return {
      ...
      ruleValidate: {
        typeDicId: [{ required: true, message: '请选择物资种类', trigger: 'blur' }],
