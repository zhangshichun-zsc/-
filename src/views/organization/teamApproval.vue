    ...
    <div class="integral-table">
      <div class="table-header table-flex">
        ...
        <div>
          ...
          <Select placeholder="排序方式" style=" margin-left:10px;width: 120px;" v-model="sort">
      ...
      <div class="pages">
        ...
        <Page :current.sync='page' :total="dataCount" show-elevator show-total size="small" :page-size="size" @on-change="changepages" />
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    getorgpage() {
      ...
      if (this.navigation1.name === 'parent') {
        ...
        orggetAuditList(this.util.remove({ ...fromobj, status: status })).then(res => {
          if (res.code == 200) {
