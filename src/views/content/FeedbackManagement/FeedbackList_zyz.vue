...
export default {
  data() {
    return {
      ...
      ruleValidate: {
        name: [{ required: true, message: '反馈类型名称不能为空', trigger: 'blur' }],
        detail: [{ required: true, message: '类型描述不能为空', trigger: 'blur' }]
      },
      modal1: false,
      navigation1: {
        head: '意见反馈列表(志愿者)'
            ...
            return h('div', [
              h(
                'a',
                {
                  ...
                  on: {
                    click: () => {
                      this.$router.push({ name: 'FeedbackDetails-Note_zyz' })
                      // this.$router.push({ name: 'FeedbackDetails-Handled_zyz' })
                      //判断是否处理并调整页面
                    }
                  }
                },
