              ...
              <ul>
                ...
                <li class="start">
                  ...
                  <div class="start-wap start-firt">
                    <UploadImg :picMap="coverMap" :max="1" v-model="cover" :display-width="120" :display-height="120" :crop-width="128" :crop-height="128" :disabled="cover.length !== 0 && isDisb" ref="refCover"></UploadImg>
                  </div>
                </li>
                <li class="start">
                  <span>活动图片</span>
                  <div class="start-wap">
                    <UploadImg :picMap="imageMap" :max="1" v-model="image" :display-width="300" :crop-width="750" :crop-height="320" :disabled="image.length !== 0 && isDisb" ref="refImags"></UploadImg>
                  </div>
                </li>
                <li>
                  <span>活动归属团队</span>
                ...
                <li class="flex-start">
                  <span>活动时间</span>
                  <DatePicker :value="args.startAt" type="datetime" :options="options" placeholder="开始时间" :disabled="isDisb" confirm style="width: 150px" @on-change="handleChange(0, 'startAt', $event)" @on-open-change="successOk(0, $event)" format="yyyy-MM-dd HH:mm" />
                  <i>&nbsp;~&nbsp;</i>
                  <DatePicker :value="args.endAt" type="datetime" :options="options" :disabled="isDisb" placeholder="结束时间" @on-change="handleChange(0, 'endAt', $event)" @on-open-change="successOk(1, $event)" confirm style="width: 150px" format="yyyy-MM-dd HH:mm" />
                </li>
                <li class="flex-start">
                  <span>招募时间</span>
                  <DatePicker :value="zhaStart" type="datetime" :options="options" :disabled="isDisb" placeholder="开始时间" confirm style="width: 150px" @on-change="handleChange(1, 'zhaStart', $event)" @on-open-change="successOk(2, $event)" format="yyyy-MM-dd HH:mm" />
                  <i>&nbsp;~&nbsp;</i>
                  <DatePicker :value="zhaEnd" :disabled="isDisb" type="datetime" :options="options" placeholder="结束时间" @on-change="handleChange(1, 'zhaEnd', $event)" @on-open-change="successOk(3, $event)" confirm style="width: 150px" format="yyyy-MM-dd HH:mm" />
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    separation() {
      let args = this.args
      ;(this.train = args.trainComments || null), (this.feed = args.coDetailList)
      this.spliced()
    },
    spliced() {
      if (this.isEdit !== 2) {
